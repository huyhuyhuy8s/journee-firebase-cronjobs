...
exports.onUserCreated = onDocumentCreated(
    ...
    async (event) => {
      ...
      if (!userData) {
        console.error("No new user data found:", userId);
        throw new HttpsError(
            "internal",
            `No user data found for userId: ${userId}`,
        );
      }
      ...
      try {
        ...
        if (!existingQuery.empty) {
          console.error(`User ${userId} already has a journal named ${journalName}. Skipping creation.`);
          throw new HttpsError(
              "already-exists",
              `Journal named ${journalName} already exists for userId: ${userId}`,
          );
        }
        ...
        if (!getJwtSecret()) {
          ...
          throw new HttpsError(
              "failed-precondition",
              "Missing JWT_SECRET in functions environment",
          );
        }
        ...
        if (!response.ok) {
          console.error("Failed to create journal for user");
          throw new HttpsError(
              "internal",
        ...
        console.error("Error creating journal for new user:", error);
        throw new HttpsError(
            "internal",
            `Failed to create journal for new user: ${error.message}`,
        );
      }
    },
);
...
exports.createJournalForUser = onCall(
    ...
    // Check authentication
      if (!request.auth) {
        console.error("Unauthenticated request to createJournalForUser");
        throw new HttpsError(
            "unauthenticated",
      ...
      try {
        ...
        if (!getJwtSecret()) {
          console.error("Error 412: Server missing JWT secret");
          throw new HttpsError(
              "failed-precondition",
              "Server is not properly configured to create journals",
          );
        }
        ...
        if (!response.ok) {
          ...
          console.error("Error 500:", `Backend error: ${response.status} ${text}`);
          throw new HttpsError(
              "internal",
        ...
        if (!journal) {
          console.error("No journal data found for user");
          throw new HttpsError(
              "internal",
              "No journal data returned from backend",
          );
        }
      ...
      try {
        ...
        for (const userDoc of users) {
          ...
          // Check if the user already has a journal
            ...
            if (!response.ok) {
              const text = await response.text();
              console.error(`Failed to create journal for ${userId}: ${response.status} ${text}`);
              throw new HttpsError(
                  "internal",
                  `Backend error: ${response.status} ${text}`,
              );
            ...
            console.error(`Error creating journal for user ${userId}:`, error);
            throw new HttpsError(
                "internal",
                `Failed to create journal for user ${userId}: ${error.message}`,
            );
          }
        }
